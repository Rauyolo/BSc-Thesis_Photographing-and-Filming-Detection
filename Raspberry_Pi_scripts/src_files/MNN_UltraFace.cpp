...
UltraFace::~UltraFace() {
    if (ultraface_interpreter != nullptr){
        ultraface_interpreter->releaseModel();
        ultraface_interpreter->releaseSession(ultraface_session);
    }
}

int UltraFace::init(int input_width, int input_length, int num_thread_,
...
int UltraFace::loadModel(const std::string &mnn_path){
    ...
    std::cout << "UltraFace model initialized..." <<std::endl;
