...
def load_datafile(data_path):
    # Hyperparameters that need to be configured
    cfg = {"model_name":None,
        ...
        for line in f.readlines():
            if line == '\n' or line[0] == "[":
                ...
                if data[0] in cfg:
                    ...
                    print("%There is an invalid configuration item in the configuration file:%s"%(data_path, data))
    return cfg
...
def evaluation(val_dataloader, cfg, model, device, conf_thres = 0.01, nms_thresh = 0.4, iou_thres = 0.5):
    ...
    for imgs, targets in pbar:
        ...
        # Perform nms processing on the predicted anchorbox
        with torch.no_grad():
            preds = model(imgs)

            # Feature map post-processing: generate anchorbox
